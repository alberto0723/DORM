...
Root sets (those without a parent) can contain other sets, but these cannot nest others.
...
usage: queryExecutor.py [--help] [--logging] [--show_sql] [--hide_progress] [--hide_warnings] --paradigm <prdgm> [--dbconf_file <conf>] [--dbschema <sch>]
                        [--query_file <path>] [--print_rows] [--print_counter] [--print_time] [--print_cost] [--save_cost] [--cost_file <path>]
...
options:
  ...
  --save_cost           Saves the costs of the queries in CSV format
  --cost_file <path>    File name to store the costs (if not provided, it uses that of the queries, just different extension)
```
...
Also, a second source schema called `source2` will be automatically created following the same steps as above, but using the design [1NF/book-authors-topic](files/designs/1NF/book-authors-topic.json) and the corresponding data in [book-authors-topic](files/data/book-authors-topic.sql).